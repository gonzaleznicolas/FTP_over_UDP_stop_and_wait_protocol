...
public class Segment {
	...
	public Segment(byte[] bytes) {
		setBytes(bytes);
	    // you might be wondering how it determines the sequence number? Go look at the setBytes function.
	    // it takes the first 4 bytes of the input srray as the integer which is the sequence number
	}
	...
	public Segment(DatagramPacket packet) {
		...
		// the this() keyword invokes another constructor. so the Arrays.copyof() returns us a byte array.
		// by putting that in a this(), it is calling a constructor which only takes a byte array as an argument. and thats
		// the constructor right above this one.
	}
